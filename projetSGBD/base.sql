...
drop table S_INSCRIT cascade constraints;
drop table TRAJET_INTERMEDIAIRE cascade constraints;
...
(
    ...
    DISTANCE			    NUMBER(4)			   ,
    constraint pk_s_inscrit primary key (NUMERO_ANNONCE,ADRESSE_MAIL,VILLE_DEPART,VILLE_ARRIVEE,DUREE_ESTIMEE,DISTANCE)
);
...
(
NUMERO_VOITURE		NUMBER(4)		not null,
...
ADRESSE_MAIL		CHAR(30)			not null,
NUMERO_ANNONCE		NUMBER(8)			not null,
constraint pk_participe_au_voyage primary key (NUMERO_ANNONCE, ADRESSE_MAIL)	
);
...
alter table S_INSCRIT
    add constraint fk1_s_inscrit foreign key (ADRESSE_MAIL)
       references ELEVE (ADRESSE_MAIL);

alter table S_INSCRIT
    add constraint fk2_s_inscrit foreign key (NUMERO_ANNONCE)
       references ANNONCE (NUMERO_ANNONCE);

alter table S_INSCRIT
    add constraint fk3_s_inscrit foreign key (VILLE_DEPART,VILLE_ARRIVEE,DUREE_ESTIMEE,DISTANCE)
       references TRAJET_INTERMEDIAIRE (VILLE_DEPART,VILLE_ARRIVEE,DUREE_ESTIMEE,DISTANCE);

--alter table S_INSCRIT
--    add constraint fk4_s_inscrit foreign key (VILLE_ARRIVEE)
--       references TRAJET_INTERMEDIAIRE (VILLE_ARRIVEE);

--alter table S_INSCRIT
--    add constraint fk5_s_inscrit foreign key (DUREE_ESTIMEE)
--       references TRAJET_INTERMEDIAIRE (DUREE_ESTIMEE);

--alter table S_INSCRIT
--    add constraint fk6_s_inscrit foreign key (DISTANCE)
...
alter table CONTIENT
    add constraint fk2_contient foreign key (VILLE_DEPART,VILLE_ARRIVEE,DUREE_ESTIMEE,DISTANCE)
       references TRAJET_INTERMEDIAIRE (VILLE_DEPART,VILLE_ARRIVEE,DUREE_ESTIMEE,DISTANCE);
...
alter table PARTICIPE_AU_VOYAGE
